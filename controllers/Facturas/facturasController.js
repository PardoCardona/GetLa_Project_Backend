  ...
  try {
    ...
    for (let i = 0; i < productos.length; i++) {
      ...
      const detalle = await new CuerpoFactura({
        ...
        referenciaProducto: p.referencia || producto.referencia || "",
    ...
    const facturaPopulada = await Factura.findById(factura._id)
      .select(
        "cabecera cliente cuerpo subtotal descuento iva total numeroFactura nombreUsuario rolUsuario createdAt",
      )
